...
# mountain, desert, plains, water
grid = []

biome_colours = [(120, 120, 120), (210, 180, 80), (80, 180, 80),(70, 130, 180)]
...
class Tile:
    def __init__(self, x, y, size, stats):
        ...
        self.stats = stats
        self.food_regen, self.fertility, self.roughness = stats
        self.stats = stats
        self.colour = (0, 0, 0)
        self.type = "generic"
    ...
    def change_stat(self, stat_to_change):
        ...
        self.food_regen = self.stats[0]
        self.fertility = self.stats[1]
        self.roughness = self.stats[2]
        self.update_colour()
...
def create_tile(x, y, size):
    ...
    stats = [random.uniform(0.1, 1.0)**0.8, random.uniform(0.1, 1.0), random.uniform(0, 1)]
    return create_tile_from_stats(x, y, size, stats)

class MountainTile(Tile):
    def __init__(self, x, y, size, stats):
        super().__init__(x, y, size, stats)
        self.colour = (120, 120, 120)
        self.type = "mountains"
...
def make_random_blobs(amnt, type, size):
    coords = []
    sizes = []
    for i in range(amnt):
        x_i = random.uniform(0, width - 1)
        y_i = random.uniform(0, height -1)
        coords.append([x_i, y_i])
        colour = find_colour(type)
        draw_blob(x_i, y_i, colour, size)


def draw_blob(cx, cy, colour, radius):
    tiles = get_tiles_in_radius(cx, cy, radius)

    for tile in tiles:
        tile.colour = colour

def get_tiles_in_radius(cx, cy, radius):
    tiles = []
    qtr = random.randint(1,5)
    chance = random.uniform(0,1)
    center_x = int(cx // tile_size)
    center_y = int(cy // tile_size)
    radius_tiles = int(radius)
    ...
    return tiles
def find_colour(type):
    if type == "mountains":
        return biome_colours[0]
    elif type == "desert":
        return biome_colours[1]
    elif type == "plain":
        return biome_colours[2]
    else:
        return biome_colours[3]
    
for y in range(rows):
    row = []
    for x in range(cols):
        row.append(create_tile(x * tile_size, y * tile_size, tile_size))
    grid.append(row)
        ...
        for c in range(4):   # region col (0–3)
            ...
            for y in range(r * region_rows, (r + 1) * region_rows):
                for x in range(c * region_cols, (c + 1) * region_cols):
                    ...
                    grid[y][x] = create_tile_from_stats(tile.x, tile.y, tile.size, tile.stats)
                    # recalc colour
                    tile.update_colour()
...
for i in range(rows*cols):
    ...
    if 0 < row_i < rows - 1 and 0 < col_i < cols - 1:
        ...
        total = np.zeros(3)
        avg = np.zeros(3)
        for dy in range(dy_range[0], dy_range[1]):   # -1, 0, 1
            for dx in range(dx_range[0], dx_range[1]):
                for m in range(0,3):
                    total[m] += grid[row_i + dy][col_i + dx].stats[m]  # pick stat index
        for n in range(0,3):
            avg[n] = total[n] / len(total)
